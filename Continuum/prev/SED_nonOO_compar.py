...
# print("CHECK Sigma_a",np.max(Sigma_a - ZSED.Sigma_a))

print("CHECK kappas",np.max(kappas - ZSED.kappa_as_abs))
print("CHECK kappas scat",np.max(kappas_scat - ZSED.kappa_as_scat))
print("CHECK tau",np.max(tau - ZSED.tau))
print("CHECK tau",np.max(tau_scat - ZSED.tau_scat))
print("CHECK tau",np.max(tau_abs - ZSED.tau_abs))



plt.figure(figsize=(10, 4))
plt.plot(nus / 1E9, Inus, label=r'$I_\nu$')
plt.plot(ZSED.nus / 1E9, ZSED.Inus, label=r'OO $I_\nu$')
#plt.plot(nus / 1E9, Inus[-1] * (nus / nus[-1])**2, label=r'$\nu^2$')
plt.ylabel('Jy/sr')
plt.xlabel(r'$\nu$ / GHz')
#plt.xscale('log')
